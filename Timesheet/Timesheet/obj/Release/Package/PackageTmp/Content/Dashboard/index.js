...
function AprovarOuReprovarLancamento(status) {
    console.log('\nFuncao de aprovar ou reprovar foi chamada!')
    $('.modal-body-usuario').remove();
    showLoading();
    $.ajax({
        ...
        success: function () {
            if (status == 1) {
                createAlert('Reprovação Registrada!', 'Sua reprovação foi registrada com sucesso.', '', 'info', true, false, 'pageMessages');
            } else {
                createAlert('Aprovação Registrada!', 'Sua aprovação foi registrada com sucesso.', '', 'success', true, false, 'pageMessages');
            }
